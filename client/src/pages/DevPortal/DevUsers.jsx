...
export default function DevUsers() {
  ...
  const menteeCount = users.filter(u => u.role !== 'mentor').length;

  const filtered = users.filter(u => {
    if (filterRole === 'mentor' && u.role !== 'mentor') return false;
    if (filterRole === 'mentee' && u.role === 'mentor') return false;
      ...
      <div className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          <p className="text-sm text-stone-400">
            {filtered.length} of {users.length} · {mentorCount} mentors · {menteeCount} mentees
          </p>
        ...
        <div className="flex gap-1">
          {[['', `All (${users.length})`], ['mentor', `Mentors (${mentorCount})`], ['mentee', `Mentees (${menteeCount})`]].map(([val, label]) => (
            <button
              key={val}
