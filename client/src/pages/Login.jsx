...
function LoginAlreadySignedIn({ user }) {
  ...
  return (
    <FuturisticAuthFrame
      mode="login"
      title={t('auth.alreadyInTitle', 'You are already in')}
      subtitle={`${t('auth.alreadyInSubtitlePrefix', 'Signed in as')} ${display}. ${t('auth.alreadyInSubtitleSuffix', 'Continue to Bridge without authenticating again.')}`}
    >
      <Link to={navigateTo} className="group inline-flex w-full items-center justify-center gap-2 rounded-2xl bg-stone-950 px-5 py-4 text-sm font-black text-white shadow-[0_18px_45px_-22px_color-mix(in srgb, var(--color-secondary) 90%, transparent)] transition hover:-translate-y-0.5 hover:bg-stone-800">
        {t('auth.continueBridge', 'Continue to Bridge')}
  ...
  return (
    <>
      <FuturisticAuthFrame
        ...
        subtitle={redirectPath ? `${t('auth.loginRedirectPrefix', 'Sign in and we will send you back to')} ${redirectPath}.` : t('auth.loginSubtitle', 'Step back into your mentor network with a focused, secure, beautifully crafted gateway.')}
        onSocialAuth={(provider) => setError(`${provider} ${t('auth.socialSigninNotConnectedSuffix', 'sign-in is not connected yet.')}`)}
        footer={<p className="text-center text-sm text-stone-500">{t('auth.noAccount', 'No account?')} <Link to="/register" className="font-black text-stone-950 underline decoration-amber-300 decoration-2 underline-offset-4 hover:text-amber-700">{t('auth.createOne', 'Create one')} →</Link></p>}
      >
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="group">
            <label htmlFor="login-email" className={labelClass}>{t('auth.emailAddress', 'Email address')}</label>
          ...
          <button type="submit" disabled={submitting} className="group relative inline-flex w-full items-center justify-center gap-2 overflow-hidden rounded-2xl bg-stone-950 px-5 py-4 text-sm font-black text-white shadow-[0_20px_48px_-24px_color-mix(in srgb, var(--color-secondary) 95%, transparent)] transition hover:-translate-y-0.5 hover:bg-stone-800 disabled:pointer-events-none disabled:opacity-55">
            ...
            <span className="relative flex items-center gap-2">{submitting ? <><span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" />{t('auth.signingIn', 'Signing in…')}</> : <>{t('auth.signIn', 'Sign In')}<ArrowRight className="h-4 w-4 transition group-hover:translate-x-0.5" /></>}</span>
          </button>
        </form>
