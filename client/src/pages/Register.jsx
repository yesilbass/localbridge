...
import { AlertCircle, ArrowRight, Eye, EyeOff, Github, Lock, Mail, Quote, User as UserIcon } from 'lucide-react';
...
function passwordStrength(pw) {
  ...
  return {
    score,
    label: ['Enter password', 'Weak', 'Fair', 'Good', 'Strong 🔒'][score],
    color: ['bg-stone-200', 'bg-red-400', 'bg-orange-400', 'bg-amber-400', 'bg-emerald-500'][score],
  };
}
  ...
  return (
    ...
    >
      ...
      <p className="relative z-10 text-xs" style={{ color: 'rgba(255,255,255,0.45)' }}>
        &copy; {new Date().getFullYear()} Bridge
      </p>
    </div>
  );
}
...
export default function Register() {
  ...
  function handleSocial(name) {
    setError(`${name} sign-up is not connected yet.`);
  }
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5">
              {error ? (
                <div
                  ...
                  style={{
                    borderColor: 'color-mix(in srgb, #ef4444 35%, var(--bridge-border))',
                    backgroundColor: 'color-mix(in srgb, #ef4444 8%, var(--bridge-surface))',
                    color: '#ef4444',
                  }}
                >
              ...
              <div>
                <div className="flex gap-1.5">
                  {[0, 1, 2, 3].map((i) => (
                    <span
                      key={i}
                      className={`h-1 flex-1 rounded-full transition ${i < pwMeta.score ? pwMeta.color : ''}`}
                      style={i >= pwMeta.score ? { backgroundColor: 'var(--bridge-border)' } : undefined}
                    />
                  ))}
                </div>
                <p className="mt-1.5 text-xs font-medium" style={{ color: 'var(--bridge-text-muted)' }}>{pwMeta.label}</p>
              </div>
