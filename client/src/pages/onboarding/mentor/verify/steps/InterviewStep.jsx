...
export default function InterviewStep({ run, latest, onAdvance }) {
  ...
  if (isPassed) return <Done title="Interview scored" body={evaluation?.summary || 'Your interview answers were rated strongly.'} onContinue={onAdvance} />;

  return (
    <div className="flex flex-col gap-5">
      <Header
        title="3-minute domain interview"
      ...
      <Field icon={Mic} label="Claimed expertise" value={expertise} onChange={setExpertise} placeholder="e.g. Senior PM, growth + onboarding" />

      <label className="flex flex-col gap-1 text-[12px] font-semibold" style={{ color: 'var(--bridge-text-secondary)' }}>
        <span>Interview transcript (3 short Q&amp;A is enough)</span>
        <textarea
          rows={8}
          value={transcript}
      ...
      {evaluation ? (
        ...
        >
          <p className="font-bold" style={{ color: 'var(--bridge-text)' }}>AI rubric</p>
      ...
      <Actions primaryLabel={busy ? 'Scoring…' : 'Submit interview'} onPrimary={submit} disabled={busy} error={error} />
    </div>
  );
}
